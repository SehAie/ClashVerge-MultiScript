...
> 💬 **维护者备注**：如需扩展其他场景（如 GitHub 加速、Docker 镜像等），按"域名列表 + 规则生成"的模式仿照添加即可。
```
...
| 改动点 | 说明 |
|--------|------|
| 🆕 新增"使用前必读" | 明确三种使用方案（A/B/C），访客一眼看清定位 |
| 🆕 新增"切换到其他协议"章节 | 提供 **8 种协议模板**（SS / SSR / VMess / VLESS / Trojan / Hysteria / TUIC / WireGuard） |
| 🆕 新增"不想自建节点"独立章节 | 详细 3 步流程，含进阶技巧（引用订阅分组） |
| 🔄 扩充字段说明表 | 增加"是否必填"列，清楚标注 `type` / `port` 的必要性 |
| 🔄 目录重构 | 把协议切换、订阅方案单独分章，便于跳转 |
| 🔄 FAQ 新增节点报错排查 | 覆盖协议字段不匹配的常见问题 |
| 🔄 检查清单补充 | 增加"占位符已替换"项 |
...
📁 clash-verge-config/
 ├─ 📄 override.js     ← 脚本主体
 ├─ 📄 README.md       ← 本文件（合并版完整文档）
 ├─ 📄 LICENSE         ← MIT（GitHub 自动生成）
 └─ 📄 .gitignore      ← Node（GitHub 自动生成）
